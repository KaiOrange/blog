...
所以如果该标签没有写颜色等信息的时候就不会有了，这也是为什么颜色不能被继承的原因。另外对于同一个标签如果多次赋值同一个样式那么只会使用第一个，就比如我们给text标签设置了一个颜色`color:red`,然后再设置一个颜色`color:blue`，那么最终整合到style中的时候是`color:red`。我们经常用不同的类选择器去覆盖正常的样式，就比如a标签正常的样式是`a-link`的颜色，比如说是绿色的，然后当点击以后class属性是`a-link active`，其中`active`表明颜色是红色的，这种情况在web应用中运行的很好，但是在快应用中却不行。
